package com.lulan.shincolle.command;

import com.lulan.shincolle.network.S2CReactPackets;
import com.lulan.shincolle.proxy.CommonProxy;
import com.lulan.shincolle.utility.EntityHelper;
import net.minecraft.command.CommandBase;
import net.minecraft.command.CommandException;
import net.minecraft.command.ICommandSender;
import net.minecraft.entity.player.EntityPlayer;
import net.minecraft.entity.player.EntityPlayerMP;
import net.minecraft.server.MinecraftServer;
import net.minecraft.util.text.TextComponentString;
import net.minecraft.util.text.TextComponentTranslation;
import net.minecraft.world.World;

import java.util.ArrayList;
import java.util.List;

public class ShipCmdShipAttrs
extends CommandBase {
    private static final ArrayList<String> Aliases = new ArrayList<>();
    static {
            Aliases.add("shipattrs");
    }

    public String getName() {
        return Aliases.get(0);
    }

    public List<String> getAliases() {
        return Aliases;
    }

    public String getUsage(ICommandSender sender) {
        return "/shipattrs <ship level> [<Bonus:HP> <Bonus:ATK> <Bonus:DEF> <Bonus:ATK Speed> <Bonus:Move Speed> <Bonus:Hit Range>]";
    }

    public boolean checkPermission(MinecraftServer server, ICommandSender sender) {
        return sender instanceof EntityPlayer;
    }

    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        World world = sender.getEntityWorld();
        EntityPlayer op;
        int senderEID;
        boolean isOP;
        if (!world.isRemote && sender instanceof EntityPlayer) {
            op = (EntityPlayer)sender;
            senderEID = op.getEntityId();
            isOP = EntityHelper.checkOP(op);
            if (args.length != 7 && args.length != 1) {
                sender.sendMessage(new TextComponentString(this.getUsage(sender)));
                sender.sendMessage(new TextComponentTranslation("chat.shincolle:command.wrongparanum").appendText(" (1 or 7)"));
                return;
            }
            if (isOP) {
                if (args.length == 1) {
                    int level = ShipCmdShipAttrs.parseInt(args[0]);
                    if (level <= 0 || level > 150) {
                        sender.sendMessage(new TextComponentString(this.getUsage(sender)));
                        sender.sendMessage(new TextComponentTranslation("chat.shincolle:command.wrongpararange").appendText(" (LV =  1 ~ 150)"));
                        return;
                    }
                    CommonProxy.channelI.sendTo(new S2CReactPackets((byte)2, senderEID, level), (EntityPlayerMP)op);
                } else {
                    int[] data = new int[7];
                    for (int i = 0; i < 7; ++i) {
                        data[i] = ShipCmdShipAttrs.parseInt(args[i]);
                    }
                    if (data[0] <= 0 || data[0] > 150) {
                        sender.sendMessage(new TextComponentString(this.getUsage(sender)));
                        sender.sendMessage(new TextComponentTranslation("chat.shincolle:command.wrongpararange").appendText(" (LV = 1 ~ 150)"));
                        return;
                    }
                    if (data[1] < 0 || data[2] < 0 || data[3] < 0 || data[4] < 0 || data[5] < 0 || data[6] < 0 || data[1] > 100 || data[2] > 100 || data[3] > 100 || data[4] > 100 || data[5] > 100 || data[6] > 100) {
                        sender.sendMessage(new TextComponentString(this.getUsage(sender)));
                        sender.sendMessage(new TextComponentTranslation("chat.shincolle:command.wrongpararange").appendText(" (Bonus = 1 ~ 100)"));
                        return;
                    }
                    CommonProxy.channelI.sendTo(new S2CReactPackets((byte)2, senderEID, data[0], data[1], data[2], data[3], data[4], data[5], data[6]), (EntityPlayerMP)op);
                }
            } else {
                sender.sendMessage(new TextComponentTranslation("chat.shincolle:command.notop"));
            }
        }
    }
}

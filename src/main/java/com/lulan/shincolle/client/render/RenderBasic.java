...
public abstract class RenderBasic extends RenderLiving<EntityLiving> {
    ...
    protected RenderBasic(RenderManager rm) {
        super(rm, null, 1.0f);
    }

    protected abstract void setModel();

    protected abstract void setMiscModel();
    ...
    public void doRender(EntityLiving entity, double x, double y, double z, float yaw, float parTick) {
        ...
        if (this.hasMiscModel() && this.miscModelList != null && !this.miscModelList.isEmpty()) {
            for (int i = 0; i < this.miscModelList.size(); ++i) {
                if (this.mainModel instanceof ShipModelBaseAdv && !((ShipModelBaseAdv)this.mainModel).shouldRenderMiscModel(i)) continue;
                this.doRenderMisc(entity, i, x, y, z, parTick);
            }
        }
    }
    ...
    protected void doRenderMisc(EntityLiving host, int miscID, double x, double y, double z, float parTick) {
        ...
        try {
            ...
            GlStateManager.scale(mm.scale.x, mm.scale.y, mm.scale.z);
            GlStateManager.rotate(mm.rotX, 1.0f, 0.0f, 0.0f);
            GlStateManager.rotate(mm.rotY, 0.0f, 1.0f, 0.0f);
            GlStateManager.rotate(mm.rotZ, 0.0f, 0.0f, 1.0f);
            ...
            if (entity instanceof EntityLiving) {
                this.renderLayers(host, f6, f5, parTick, f8, f2, f7, f4);
            }
